...
def _visualize_alias(data):
    """兼容画布里的 input_url/output_url 参数，生成同一份多维图表页面。"""
    data = dict(data)
    if data.get("input_url") and not data.get("clean_csv_url"):
        data["clean_csv_url"] = data.get("input_url")
    result = multi_chart_render(data)
    chart_url = result.get("data", {}).get("chart_url")
    if chart_url:
        result["data"]["output_url"] = chart_url
    return result


def analyze_city(data):
    return _visualize_alias(data)
...
def analyze_subclass(data):
    return _visualize_alias(data)


def analyze_price_type(data):
    return _visualize_alias(data)
